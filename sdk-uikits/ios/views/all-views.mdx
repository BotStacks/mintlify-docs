---
title: "iOS Views"
sidebarTitle: "All Views"
description: ""
icon: "images"
iconType: "solid"
---

Our SDK at BotStacks includes convenient screen content views, which are essentially a multiple components working together under the hood, generally using a `State`.

The views rely on a `State` object for driving UI and handling asynchronous updates for saving, creating, and updating. The `state` will be a required parameter on each View.

They generally can be used for the entire contents on a `screen` and are drop in capable.

Outlined below are the content views currently available and examples on how to use them.

<CardGroup cols={2}>

{" "}
<Card
  title="Channel Settings"
  icon="gear"
  iconType="solid"
  href="channel-settings"
></Card>

{" "}
<Card
  title="Create Channel"
  icon="circle-plus"
  iconType="solid"
  href="create-channel"
></Card>

{" "}
<Card
  title="Edit Profile"
  icon="user-pen"
  iconType="solid"
  href="edit-profile"
></Card>

{" "}
<Card
  title="Select Channel Users"
  icon="users"
  iconType="solid"
  href="select-channel-users"
></Card>

{" "}
<Card
  title="User Details"
  icon="address-card"
  iconType="solid"
  href="user-details"
></Card>

</CardGroup>
